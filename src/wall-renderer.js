...
export function drawWall(renderer, x, y, camX, camY) {
    ...
    if (isTopGate || isBottomGate || isLeftGate || isRightGate) {
        ...
        // Draw Steel Gate Texture (Behind the wall cutout)
        if (isTopGate || isBottomGate) {
            ctx.drawImage(ASSETS.gate, 0, 0, drawWidth, tileSize);
        }
...
export function drawGateOverlay(renderer, y, camX, camY) {
    ...
    for (let x = 0; x < CONFIG.GRID_W; x++) {
        ...
        // Include only neighbors NEAR the North gate on the North row (y=0)
        const isNorthRowNearGate = (y === 0 && Math.abs(x - gateX) <= 2);
        const shouldOverlay = isLeftGate || isRightGate || isTopGate || isBottomGate || isNorthRowNearGate;
