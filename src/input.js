...
export const Input = {
    ...
    active: true,
    onTap: null, // Callback for taps (x, y)
    ...
    init(canvas) {
        ...
        window.addEventListener('mouseup', (e) => {
            if (e.button === 0) {
                this.mouse.leftDown = false;
                // Trigger tap callback on click release if it wasn't a drag
                // For simplicity in this env, we just treat click as tap
                if (this.onTap && !this.joystickVector.x && !this.joystickVector.y) {
                    if (e.target === canvas) {
                        const pos = getMappedCoordinates(e.clientX, e.clientY);
        ...
        canvas.addEventListener('touchend', (e) => {
            ...
            this.mouse.leftDown = false;
            // Handle Touch Tap
            // If touch didn't move much (we'd need to track start pos, but for now simple trigger)
            // Using last known mouse pos from touchmove/start
            if (this.onTap) {
                 this.onTap(this.mouse.x, this.mouse.y);
            }
        });
    },
